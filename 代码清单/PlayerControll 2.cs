...
public class PlayerControll : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (scene.name == "Running2" && timer > 0)
        {
    ...
    {
        Scene scene = SceneManager.GetActiveScene();
        if (scene.name == "Running2" && isGround == false && Hp > 0 && count==1)
        {
            //给一个力  方向*数值
            rBody.AddForce(Vector2.up * 400);
            //播放音效
    ...
    void Again()
    {
        ...
        Scene scene = SceneManager.GetActiveScene();
        if (scene.name=="Running")
        {
            Application.LoadLevel("Running");
        }
        else if(scene.name == "Running2")
        {
            Application.LoadLevel("Running2");
        }
